# 组播协议测试

<cite>
**本文档引用的文件**
- [main.go](file://main.go)
- [root.go](file://cmd/root.go)
- [send.go](file://cmd/send.go)
- [recv.go](file://cmd/recv.go)
- [config_cmd.go](file://cmd/config_cmd.go)
- [config.go](file://internal/config/config.go)
- [protocol.go](file://internal/proto/protocol.go)
- [ssm.go](file://internal/proto/ssm.go)
- [packet.go](file://internal/packet/packet.go)
- [go.mod](file://go.mod)
</cite>

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考虑](#性能考虑)
8. [故障排除指南](#故障排除指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本文件面向使用 ping-x 进行组播协议测试的用户与开发者，系统性阐述组播测试的配置选项、工作原理与使用方法。当前仓库展示了组播测试的命令行接口、配置模型与协议抽象层，但组播发送与接收的具体网络实现尚未在仓库中完全呈现。本文将基于现有代码，提供可操作的配置说明、典型使用场景与网络环境建议，帮助您正确配置和使用组播测试功能。

## 项目结构
项目采用模块化分层设计：
- 应用入口与命令行：main.go 负责版本注入与启动；cmd 包下的 root、send、recv、config_cmd 提供 CLI 子命令与参数解析。
- 配置模型：internal/config 定义测试配置结构与校验逻辑。
- 协议抽象：internal/proto 定义 Sender/Receiver 接口及协议工厂，预留 MulticastSender/MulticastReceiver、SSMSender/SSMReceiver 等实现占位。
- 数据包模型：internal/packet 定义探测包结构与序列化/反序列化能力。
- 依赖管理：go.mod 描述外部依赖，包括 Cobra（CLI）、Viper（配置）与 YAML 解析等。

```mermaid
graph TB
A["main.go<br/>应用入口"] --> B["cmd/root.go<br/>根命令与全局配置"]
B --> C["cmd/send.go<br/>发送子命令"]
B --> D["cmd/recv.go<br/>接收子命令"]
B --> E["cmd/config_cmd.go<br/>示例配置生成"]
C --> F["internal/config/config.go<br/>配置模型与校验"]
D --> F
E --> F
C --> G["internal/proto/protocol.go<br/>协议工厂"]
D --> G
G --> H["internal/proto/ssm.go<br/>SSM 占位实现"]
C --> I["internal/packet/packet.go<br/>探测包模型"]
D --> I
```

**图表来源**
- [main.go:1-11](file://main.go#L1-L11)
- [root.go:15-25](file://cmd/root.go#L15-L25)
- [send.go:11-32](file://cmd/send.go#L11-L32)
- [recv.go:11-27](file://cmd/recv.go#L11-L27)
- [config_cmd.go:10-20](file://cmd/config_cmd.go#L10-L20)
- [config.go:11-27](file://internal/config/config.go#L11-L27)
- [protocol.go:21-35](file://internal/proto/protocol.go#L21-L35)
- [ssm.go:11-20](file://internal/proto/ssm.go#L11-L20)
- [packet.go:53-88](file://internal/packet/packet.go#L53-L88)

**章节来源**
- [main.go:1-11](file://main.go#L1-L11)
- [root.go:15-25](file://cmd/root.go#L15-L25)
- [send.go:11-32](file://cmd/send.go#L11-L32)
- [recv.go:11-27](file://cmd/recv.go#L11-L27)
- [config_cmd.go:10-20](file://cmd/config_cmd.go#L10-L20)
- [config.go:11-27](file://internal/config/config.go#L11-L27)
- [protocol.go:21-35](file://internal/proto/protocol.go#L21-L35)
- [ssm.go:11-20](file://internal/proto/ssm.go#L11-L20)
- [packet.go:53-88](file://internal/packet/packet.go#L53-L88)

## 核心组件
- 测试配置模型（TestConfig）：统一承载协议类型、模式（send/recv）、目标地址、组播组、源地址、端口、绑定地址、网络接口、发送次数、间隔、超时、包大小、TTL 等字段。
- 配置校验（Validate）：确保协议合法、模式合法、端口有效、send/recv 模式下必要字段齐全（如 multicast/ssm 需要 group，tcp/udp send 需要 target），并校验时间格式。
- 协议工厂（NewSender/NewReceiver）：按协议类型返回对应的 Sender/Receiver 实现，当前已支持 tcp、udp、multicast、ssm。
- CLI 子命令：
  - send：支持通过命令行参数或配置文件批量运行 send 模式的测试任务。
  - recv：支持通过命令行参数或配置文件批量运行 recv 模式的测试任务。
  - config：生成示例 YAML 配置文件，包含组播与 SSM 的典型配置片段。

**章节来源**
- [config.go:11-27](file://internal/config/config.go#L11-L27)
- [config.go:49-100](file://internal/config/config.go#L49-L100)
- [protocol.go:21-51](file://internal/proto/protocol.go#L21-L51)
- [send.go:11-32](file://cmd/send.go#L11-L32)
- [send.go:34-91](file://cmd/send.go#L34-L91)
- [recv.go:11-27](file://cmd/recv.go#L11-L27)
- [recv.go:29-72](file://cmd/recv.go#L29-L72)
- [config_cmd.go:22-85](file://cmd/config_cmd.go#L22-L85)

## 架构总览
下图展示从命令行到配置模型与协议工厂的整体流程，以及当前仓库中已实现的组件映射。

```mermaid
sequenceDiagram
participant U as "用户"
participant M as "main.go"
participant R as "rootCmd"
participant S as "send/recv 子命令"
participant CFG as "配置模型(config)"
participant P as "协议工厂(protocol)"
U->>M : 启动程序
M->>R : 初始化并设置版本
U->>S : 执行 send/recv 命令
S->>CFG : 构建/加载测试配置
CFG-->>S : 返回配置对象
S->>CFG : 校验配置(Validate)
CFG-->>S : 校验结果
S->>P : 根据协议类型创建 Sender/Receiver
P-->>S : 返回具体实现(占位/待实现)
S-->>U : 输出运行信息(占位)
```

**图表来源**
- [main.go:7-10](file://main.go#L7-L10)
- [root.go:15-25](file://cmd/root.go#L15-L25)
- [send.go:34-91](file://cmd/send.go#L34-L91)
- [recv.go:29-72](file://cmd/recv.go#L29-L72)
- [config.go:49-100](file://internal/config/config.go#L49-L100)
- [protocol.go:21-51](file://internal/proto/protocol.go#L21-L51)

## 详细组件分析

### 组件一：测试配置模型与校验
- 字段覆盖：协议类型、模式、目标、组播组、源、端口、绑定地址、网络接口、计数、间隔、超时、包大小、TTL。
- 校验规则：
  - 协议必须为 tcp、udp、multicast 或 ssm。
  - 模式必须为 send 或 recv。
  - 端口范围 1-65535。
  - send 模式下：
    - tcp/udp 需要 target。
    - multicast/ssm 需要 group。
  - recv 模式下：
    - multicast/ssm 需要 group。
  - 间隔与超时需能解析为时间格式。

```mermaid
flowchart TD
Start(["开始"]) --> Proto["校验协议类型"]
Proto --> Mode["校验模式(send/recv)"]
Mode --> Port["校验端口范围"]
Port --> SendMode{"是否为 send 模式?"}
SendMode --> |是| CheckTarget["tcp/udp 是否有 target"]
SendMode --> |是| CheckGroupSend["multicast/ssm 是否有 group"]
SendMode --> |否| Next1["跳过 send 特殊校验"]
CheckTarget --> Next2["继续"]
CheckGroupSend --> Next2
Next1 --> Next2
Next2 --> RecvMode{"是否为 recv 模式?"}
RecvMode --> |是| CheckGroupRecv["multicast/ssm 是否有 group"]
RecvMode --> |否| TimeFmt["校验 interval/timeout 时间格式"]
CheckGroupRecv --> TimeFmt
TimeFmt --> End(["结束"])
```

**图表来源**
- [config.go:49-100](file://internal/config/config.go#L49-L100)

**章节来源**
- [config.go:11-27](file://internal/config/config.go#L11-L27)
- [config.go:49-100](file://internal/config/config.go#L49-L100)

### 组件二：命令行参数与配置文件驱动
- send 子命令：
  - 支持通过命令行参数直接运行，或通过配置文件批量运行 send 任务。
  - 关键参数：proto、target、group、port、bind、iface、count、interval、timeout、size、ttl。
- recv 子命令：
  - 支持通过命令行参数直接运行，或通过配置文件批量运行 recv 任务。
  - 关键参数：proto、bind、port、group、source、iface。
- config 子命令：
  - 生成包含组播与 SSM 示例的 YAML 配置文件，便于快速上手。

```mermaid
sequenceDiagram
participant U as "用户"
participant SEND as "send 子命令"
participant CFG as "配置模型"
participant FILE as "配置文件"
U->>SEND : 指定 --config 或直接传参
alt 使用配置文件
SEND->>FILE : 读取 YAML
FILE-->>SEND : 返回 tests 列表
loop 遍历 send 任务
SEND->>CFG : 构建 TestConfig
CFG-->>SEND : 校验结果
SEND-->>U : 输出任务信息(占位)
end
else 直接传参
SEND->>CFG : 构建 TestConfig
CFG-->>SEND : 校验结果
SEND-->>U : 输出任务信息(占位)
end
```

**图表来源**
- [send.go:34-91](file://cmd/send.go#L34-L91)
- [config_cmd.go:22-85](file://cmd/config_cmd.go#L22-L85)
- [config.go:34-47](file://internal/config/config.go#L34-L47)

**章节来源**
- [send.go:11-32](file://cmd/send.go#L11-L32)
- [send.go:34-91](file://cmd/send.go#L34-L91)
- [recv.go:11-27](file://cmd/recv.go#L11-L27)
- [recv.go:29-72](file://cmd/recv.go#L29-L72)
- [config_cmd.go:22-85](file://cmd/config_cmd.go#L22-L85)
- [config.go:34-47](file://internal/config/config.go#L34-L47)

### 组件三：协议工厂与实现占位
- 工厂函数根据协议字符串返回对应 Sender/Receiver 实现：
  - tcp：TCPSender
  - udp：UDPSender
  - multicast：MulticastSender
  - ssm：SSMSender
- 当前仓库中 MulticastSender/MulticastReceiver 与 SSMReceiver/SSMSender 仅提供接口定义与占位实现，实际网络交互逻辑尚未在此仓库中实现。

```mermaid
classDiagram
class Sender {
+Send(ctx, cfg, stat) error
}
class Receiver {
+Receive(ctx, cfg) error
}
class TCPSender
class UDPSender
class MulticastSender
class SSMSender
class TCPReceiver
class UDPReceiver
class MulticastReceiver
class SSMReceiver
Sender <|.. TCPSender
Sender <|.. UDPSender
Sender <|.. MulticastSender
Sender <|.. SSMSender
Receiver <|.. TCPReceiver
Receiver <|.. UDPReceiver
Receiver <|.. MulticastReceiver
Receiver <|.. SSMReceiver
```

**图表来源**
- [protocol.go:11-51](file://internal/proto/protocol.go#L11-L51)
- [ssm.go:11-20](file://internal/proto/ssm.go#L11-L20)

**章节来源**
- [protocol.go:21-51](file://internal/proto/protocol.go#L21-L51)
- [ssm.go:11-20](file://internal/proto/ssm.go#L11-L20)

### 组件四：数据包模型（探测包）
- 探测包包含魔术字、序号、时间戳、负载长度与可选负载。
- 反序列化时校验魔术字，计算 RTT 并支持生成确认包。

```mermaid
flowchart TD
A["输入字节流"] --> B["检查长度>=头部长度"]
B --> |否| E["返回错误: 数据过短"]
B --> |是| C["解析头部字段"]
C --> D{"魔术字匹配?"}
D --> |否| F["返回错误: 魔术字不匹配"]
D --> |是| G["复制剩余字节为负载"]
G --> H["返回探测包对象"]
```

**图表来源**
- [packet.go:53-88](file://internal/packet/packet.go#L53-L88)

**章节来源**
- [packet.go:53-88](file://internal/packet/packet.go#L53-L88)

## 依赖关系分析
- 外部依赖主要通过 Cobra（命令行框架）、Viper（配置管理）与 YAML 解析库组成。
- 项目模块与依赖关系如下：

```mermaid
graph TB
subgraph "应用层"
MAIN["main.go"]
ROOT["cmd/root.go"]
SEND["cmd/send.go"]
RECV["cmd/recv.go"]
CONF["cmd/config_cmd.go"]
end
subgraph "内部模块"
CFG["internal/config/config.go"]
PROT["internal/proto/protocol.go"]
SSM["internal/proto/ssm.go"]
PKT["internal/packet/packet.go"]
end
MAIN --> ROOT
ROOT --> SEND
ROOT --> RECV
ROOT --> CONF
SEND --> CFG
RECV --> CFG
SEND --> PROT
RECV --> PROT
PROT --> SSM
SEND --> PKT
RECV --> PKT
```

**图表来源**
- [main.go:1-11](file://main.go#L1-L11)
- [root.go:15-25](file://cmd/root.go#L15-L25)
- [send.go:11-32](file://cmd/send.go#L11-L32)
- [recv.go:11-27](file://cmd/recv.go#L11-L27)
- [config_cmd.go:10-20](file://cmd/config_cmd.go#L10-L20)
- [config.go:11-27](file://internal/config/config.go#L11-L27)
- [protocol.go:21-51](file://internal/proto/protocol.go#L21-L51)
- [ssm.go:11-20](file://internal/proto/ssm.go#L11-L20)
- [packet.go:53-88](file://internal/packet/packet.go#L53-L88)

**章节来源**
- [go.mod:1-25](file://go.mod#L1-L25)

## 性能考虑
- TTL 设置：命令行参数提供 TTL 选项，默认行为由实现决定。对于组播，TTL 决定数据包在网络中的转发范围。
- 发送频率与超时：通过 interval 与 timeout 控制发送节奏与等待确认的时间窗口，避免过度占用带宽或产生过多丢包。
- 包大小：size 参数影响 MTU 与链路拥塞情况，建议结合网络环境调整。
- 并发与资源：在大规模组播测试中，注意系统文件描述符与网络缓冲区限制。

## 故障排除指南
- 配置校验失败：
  - 协议非法：确保 proto 为 tcp、udp、multicast 或 ssm。
  - 模式非法：确保 mode 为 send 或 recv。
  - 端口无效：确保 port 在 1-65535 范围内。
  - send 模式缺少必要字段：tcp/udp 需要 target；multicast/ssm 需要 group。
  - recv 模式缺少必要字段：multicast/ssm 需要 group。
  - 时间格式错误：interval 与 timeout 需符合 Go 的 duration 格式。
- 组播不可达：
  - 确认网络设备支持 IGMP 与组播转发。
  - 检查路由器/交换机是否启用组播路由与 IGMP Snooping。
  - 防火墙策略允许相应端口与组播地址的流量。
  - 确保 iface 指定正确的网络接口名称。
- SSM 场景：
  - 确保 source 参数正确指向源地址。
  - 验证组播组与源的对应关系满足 SSM 规则。

**章节来源**
- [config.go:49-100](file://internal/config/config.go#L49-L100)
- [send.go:68-85](file://cmd/send.go#L68-L85)
- [recv.go:55-67](file://cmd/recv.go#L55-L67)

## 结论
本仓库提供了组播测试的命令行接口、配置模型与协议抽象层，能够通过命令行或配置文件驱动 send/recv 任务。尽管组播与 SSM 的具体网络实现尚未在仓库中完全呈现，但配置与校验逻辑已完备，可作为后续实现的基础。建议在使用前先参考“附录”的配置示例与网络环境要求，确保组播通信的正确性与稳定性。

## 附录

### A. 组播协议基础与概念
- 组播地址范围：224.0.0.0 至 239.255.255.255，其中 224.0.0.0/24 为链路本地组播地址，常用于协议通告。
- D 类地址：IP 地址分类中，D 类用于组播，其前四位为 1110。
- 路由器与组播：路由器通过 IGMP（Internet Group Management Protocol）与上游路由器交换组播路由信息，实现跨网段转发。
- 网络拓扑要求：需要支持 IGMP 的交换机与启用组播路由的路由器，确保组播报文能在不同网段间转发。

### B. 配置选项详解
- 协议类型（proto）：tcp、udp、multicast、ssm。
- 模式（mode）：send（发送）、recv（接收）。
- 目标（target）：tcp/udp send 模式必填。
- 组播组（group）：multicast/ssm send/recv 模式必填。
- 源（source）：ssm recv 模式必填。
- 端口（port）：必填，1-65535。
- 绑定地址（bind）：send/recv 均可使用，默认 0.0.0.0。
- 网络接口（iface）：多播时可指定接口名。
- 计数（count）：发送次数，0 表示持续发送。
- 间隔（interval）：发送间隔，支持 Go duration 格式。
- 超时（timeout）：单包超时时间，支持 Go duration 格式。
- 包大小（size）：探测包大小（字节）。
- TTL：TTL 值，0 表示自动选择（例如 unicast=64，multicast=16）。

**章节来源**
- [send.go:21-31](file://cmd/send.go#L21-L31)
- [recv.go:21-26](file://cmd/recv.go#L21-L26)
- [config.go:11-27](file://internal/config/config.go#L11-L27)

### C. 使用场景与配置示例
- 本地网络组播测试：
  - 发送端：配置 proto=multicast、mode=send、group=239.x.x.x、port=9002、iface=eth0、count=20。
  - 接收端：配置 proto=multicast、mode=recv、group=239.x.x.x、port=9002、iface=eth0。
- 跨网段组播通信验证：
  - 确保路由器启用组播路由与 IGMP Snooping。
  - 在不同网段分别部署发送端与接收端，使用相同 group 与 port。
- SSM 场景：
  - 接收端：配置 proto=ssm、mode=recv、group=232.x.x.x、source=源地址、port=9003、iface=eth0。
  - 发送端：配置 proto=ssm、mode=send、group=232.x.x.x、port=9003、iface=eth0、bind=源地址、count=20。

示例配置文件可通过 config 子命令生成，包含上述典型场景的片段。

**章节来源**
- [config_cmd.go:22-85](file://cmd/config_cmd.go#L22-L85)

### D. 网络配置与防火墙建议
- 路由器/交换机：
  - 启用 IGMP Snooping，确保组播流量仅在需要的端口转发。
  - 配置组播路由协议（如 PIM），保证跨网段转发。
- 防火墙：
  - 开放相应端口与组播地址范围的入站/出站规则。
  - 对于 SSM，确保源地址与组播组的对应关系被允许。
- 主机侧：
  - 确认网络接口名称与 IP 地址配置正确。
  - 如需限制 TTL，合理设置以控制转发范围。